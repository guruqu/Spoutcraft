...
public class GuiMultiplayer extends GuiScreen {
	...
	public void initGui() {
		...
		if (!this.field_74024_A) {
			...
			try {
				this.localServerFindThread = new ThreadLanServerFind(
						this.localNetworkServerList);
				this.localServerFindThread.start();
			} catch (Exception var2) {
				this.mc.getLogAgent().logWarning(
			...
			this.serverSlotContainer.func_77207_a(this.width, this.height, 32,
					this.height - 64);
		}
	...
	public void initGuiControls() {
		this.buttonList.add(this.field_96289_p = new GuiButton(7,
				this.width / 2 - 154, this.height - 28, 70, 20, I18n
						.getString("selectServer.edit")));
		this.buttonList.add(this.buttonDelete = new GuiButton(2,
				this.width / 2 - 74, this.height - 28, 70, 20, I18n
						.getString("selectServer.delete")));
		this.buttonList.add(this.buttonSelect = new GuiButton(1,
				this.width / 2 - 154, this.height - 52, 100, 20, I18n
						.getString("selectServer.select")));
		this.buttonList.add(new GuiButton(4, this.width / 2 - 50,
				this.height - 52, 100, 20, I18n
						.getString("selectServer.direct")));
		this.buttonList.add(new GuiButton(3, this.width / 2 + 4 + 50,
				this.height - 52, 100, 20, I18n.getString("selectServer.add")));
		this.buttonList.add(new GuiButton(8, this.width / 2 + 4,
				this.height - 28, 70, 20, I18n
						.getString("selectServer.refresh")));
		this.buttonList.add(new GuiButton(0, this.width / 2 + 4 + 76,
				this.height - 28, 75, 20, I18n.getString("gui.cancel")));
		boolean var1 = this.selectedServer >= 0
				&& this.selectedServer < this.serverSlotContainer.getSize();
	...
	protected void actionPerformed(GuiButton par1GuiButton) {
		if (par1GuiButton.enabled) {
			if (par1GuiButton.id == 2) {
				String var2 = this.internetServerList
						.getServerData(this.selectedServer).serverName;

				if (var2 != null) {
					this.deleteClicked = true;
					String var3 = I18n.getString("selectServer.deleteQuestion");
					String var4 = "\'" + var2 + "\' "
							+ I18n.getString("selectServer.deleteWarning");
					String var5 = I18n.getString("selectServer.deleteButton");
					String var6 = I18n.getString("gui.cancel");
					GuiYesNo var7 = new GuiYesNo(this, var3, var4, var5, var6,
							this.selectedServer);
				...
				this.mc.displayGuiScreen(new GuiScreenServerList(this,
						this.theServerData = new ServerData(I18n
								.getString("selectServer.defaultName"), "")));
				...
				ServerData var8 = this.internetServerList
						.getServerData(this.selectedServer);
				this.theServerData = new ServerData(var8.serverName,
						var8.serverIP);
		...
		if (par2 == 59) {
			...
			if (var3 > 0 && var3 < this.internetServerList.countServers()) {
				...
				if (var3 < this.internetServerList.countServers() - 1) {
					this.serverSlotContainer
							.func_77208_b(-this.serverSlotContainer.slotHeight);
				}
			}
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		this.drawCenteredString(this.fontRenderer,
				I18n.getString("multiplayer.title"), this.width / 2, 20,
				16777215);
	...
	private void joinServer(int par1) {
		if (par1 < this.internetServerList.countServers()) {
			...
			if (par1 < this.listofLanServers.size()) {
				...
				this.connectToServer(new ServerData(var2.getServerMotd(), var2
						.getServerIpPort()));
			}
		}
	}
	...
	private static void func_74017_b(ServerData par0ServerData)
			throws IOException {
		ServerAddress var1 = ServerAddress
				.func_78860_a(par0ServerData.serverIP);
		Socket var2 = null;
		...
		try {
			...
			var2.connect(new InetSocketAddress(var1.getIP(), var1.getPort()),
					3000);
			var3 = new DataInputStream(var2.getInputStream());
			var4 = new DataOutputStream(var2.getOutputStream());
			Packet254ServerPing var5 = new Packet254ServerPing(78,
					var1.getIP(), var1.getPort());
				...
				if (var8 >= var7.length) {
					...
					if (var6.startsWith("\u00a7") && var6.length() > 1) {
						...
						if (MathHelper.parseIntWithDefault(var10[0], 0) == 1) {
							par0ServerData.serverMOTD = var10[3];
							par0ServerData.field_82821_f = MathHelper
									.parseIntWithDefault(var10[1],
											par0ServerData.field_82821_f);
							par0ServerData.gameVersion = var10[2];
							var8 = MathHelper.parseIntWithDefault(var10[4], 0);
							var9 = MathHelper.parseIntWithDefault(var10[5], 0);

							if (var8 >= 0 && var9 >= 0) {
								par0ServerData.populationInfo = EnumChatFormatting.GRAY
										+ ""
										+ var8
										+ ""
										+ EnumChatFormatting.DARK_GRAY
										+ "/"
										+ EnumChatFormatting.GRAY + var9;
							} else {
								par0ServerData.populationInfo = ""
										+ EnumChatFormatting.DARK_GRAY + "???";
							}
						} else {
							par0ServerData.gameVersion = "???";
							par0ServerData.serverMOTD = ""
									+ EnumChatFormatting.DARK_GRAY + "???";
					...
					try {
						var8 = Integer.parseInt(var10[1]);
						var9 = Integer.parseInt(var10[2]);
					...
					if (var8 >= 0 && var9 > 0) {
						par0ServerData.populationInfo = EnumChatFormatting.GRAY
								+ "" + var8 + "" + EnumChatFormatting.DARK_GRAY
								+ "/" + EnumChatFormatting.GRAY + var9;
					} else {
						par0ServerData.populationInfo = ""
								+ EnumChatFormatting.DARK_GRAY + "???";
	...
	protected void func_74007_a(String par1Str, int par2, int par3) {
		if (par1Str != null) {
			...
			this.drawGradientRect(var4 - 3, var5 - 3, var4 + var6 + 3,
					var5 + 8 + 3, -1073741824, -1073741824);
	...
	static int getAndSetSelectedServer(GuiMultiplayer par0GuiMultiplayer,
			int par1) {
	...
	static String getAndSetLagTooltip(GuiMultiplayer par0GuiMultiplayer,
			String par1Str) {
